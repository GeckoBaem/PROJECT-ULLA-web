...
const menuButton = document.querySelector(".header-button-menu") as HTMLDivElement;
const menuLines = document.querySelectorAll(".header-button-menu hr") as NodeListOf<HTMLHRElement>;
...
const enableToggle = (delay: number) => {
    return setTimeout(() => {
        canToggle = true;
    }, delay);
}
...
const toggleMenu = (element: HTMLDivElement) => {
    if (!canToggle) return;
